...
class ProductController extends Controller
{
    ...
    public function getProductDetails(Request $request, $slug): JsonResponse
    {
        ...
        $product = Product::with(['reviews.customer', 'seller.shop', 'tags', 'digitalVariation', 'brand'])
    ...
    public function get_most_demanded_product(Request $request)
    {
        ...
        if ($products) {
            ...
            $colorImages = $productDetails['color_image'] ?? '[]';
            $baseUrl = env('APP_URL') . '/storage/app/public/product/';

            $decodedImages = json_decode($images, true);
            $decodedColorImages = json_decode($colorImages, true);

            $fullImages = array_map(function ($image) use ($baseUrl) {
                return [
                        'image_name' => $image['image_name'],
                        'full_url' => $baseUrl . $image['image_name'],
            ...
            $products['product_info'] = [
                ...
                'featured_status' => $productDetails['featured_status'] ?? '',
    ...
    public function stripe(Request $request)
    {
        ...
        try {
            ...
            $paymentIntent = \Stripe\PaymentIntent::create([
                ...
                // 'return_url' => 'https://healthhive.me/payment/success',
            ]);
    ...
    public function handleWebhook(Request $request)
    {
        ...
        try {
            ...
            if ($event->type == 'payment_intent.succeeded') {
                ...
                $total_price = $paymentIntent->metadata->total;

                $update_booked_service = Order::where('customer_id', $user_id)
                    ->orderBy('id', 'desc')
                    ->update([
                        'payment_status' => 'paid',
                ...
                Log::info("Order updated", [
                    ...
                    'update_status' => $update_booked_service
                ]);

                return response()->json(['success' => true, 'msg' => 'Payment processed successfully']);
            }
            ...
            Log::error("Webhook Error: General error", [
                'error' => $e->getMessage(),
                'trace' => $e->getTraceAsString()
            ]);
    ...
    public function saveProducts(Request $request)
    {
        ...
        try {
            ...
            $product = $saveProduct->product; // Assuming you have a `product` relationship in SaveProduct model
            $user = $saveProduct->user; // Assuming you have a `user` relationship in SaveProduct model
    ...
    {
        $validator = Validator::make($request->all(), [
            'product_id' => 'required|exists:save_products,product_id',
            'user_id' => 'required|exists:save_products,user_id',
        ], [
            'product_id.required' => 'Product ID is required.',
            'product_id.exists' => 'The provided Product ID does not exist in saved products.',
            'user_id.required' => 'User ID is required.',
            'user_id.exists' => 'The provided User ID does not exist in saved products.',
        ]);
        ...
        try {
            ...
            $savedProduct = SaveProduct::where('product_id', $request->product_id)
            ...
            if (!$savedProduct) {
                return response()->json([
                    'status' => 'error',
                    'message' => 'The saved product entry was not found.'
                ], 404);
            }
            ...
            return response()->json([
                'status' => 'error',
                'message' => 'An error occurred while removing the saved product.',
                'error' => $e->getMessage()
            ], 500);
        }
    }
    ...
    public function listSavedProducts(Request $request)
    {
        ...
        try {
            $savedProducts = SaveProduct::with('product','user')
                ->where('user_id', $request->user_id)
                ->get();
            ...
            if ($savedProducts->isEmpty()) {
                return response()->json([
                    'status' => 'success',
                    'message' => 'No saved products found for this user.',
                    'data' => []
                ], 200);
            }
            ...
            return response()->json([
                'status' => 'error',
                'message' => 'An error occurred while fetching saved products.',
                'error' => $e->getMessage()
            ], 500);
        }
    }
