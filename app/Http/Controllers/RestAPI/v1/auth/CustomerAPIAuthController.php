...
class CustomerAPIAuthController extends Controller
{
    ...
    public function register(Request $request): JsonResponse
    {
        $validator = Validator::make($request->all(), [
            ...
            'study_duration'=>'nullable', 
            'dob'=>'nullable',
            'interests'=>'nullable',
            'gender'=>'nullable',
            // End-student=feilds
            // 'university_card_or_fee_payment' => 'nullable',
            'university_card_or_fee_payment' => 'nullable|file|mimes:jpg,jpeg,png,pdf',
            // For vendor
            'trade_license_number' => 'nullable|min:3',
            'national_id_number'=>'nullable|min:3',
            // For Health care worker
            'gpa'=>'nullable',
            'graduation_date'=>'nullable',
            'feild_of_work'=>'nullable',
            'passport_or_national_id'=>'nullable|file|mimes:jpg,jpeg,png,pdf',
            // Muhammad-Sufyan-New-feilds-for-product-recommendation
            ...
            'hospital_logo'=>'nullable|file|mimes:jpg,jpeg,png,pdf',
        ], [
        ...
        $filePath = $request->file('university_card_or_fee_payment') ? $request->file('university_card_or_fee_payment')->move(public_path('images'), $request->file('university_card_or_fee_payment')->getClientOriginalName()) : null;
        $passportFilePath = $request->file('passport_or_national_id') ? $request->file('passport_or_national_id')->move(public_path('images'), $request->file('passport_or_national_id')->getClientOriginalName()) : null;
        $hospitalLogo = $request->file('hospital_logo') ? $request->file('hospital_logo')->move(public_path('images'), $request->file('hospital_logo')->getClientOriginalName()) : null;
        
    
        // Generate URLs relative to public folder
        $filePathUrl = $filePath ? url('public/images/' . $request->file('university_card_or_fee_payment')->getClientOriginalName()) : null;
        $passportFilePathUrl = $passportFilePath ? url('public/images/' . $request->file('passport_or_national_id')->getClientOriginalName()) : null;
        $hospitalLogoUrl = $hospitalLogo ? url('public/images/' . $request->file('hospital_logo')->getClientOriginalName()) : null;
        ...
        $user = User::create([
            ...
            'university_card_or_fee_payment' => $filePathUrl,
            'is_student' => $request->is_student,
            // Muhammad-Sufyan-New-feilds-for-product-recommendation
            'study_start_year'=>$request->study_start_year,
            ...
            'feild_of_work'=>$request->feild_of_work,
            'passport_or_national_id'=>$passportFilePathUrl,
            'is_health_care_worker'=>$request->is_health_care_worker,
            // Muhammad-Sufyan-New-feilds-for-product-recommendation
        ...
        return response()->json(['success' => true, 'msg' => 'OTP sent to your email address. Verify it to continue.', 'data' => $user], 200);
    }
    ...
    public function verification($id)
    {
        ...
        if(!$user || $user->is_email_verified == 1){
            return response()->json(['success' => true, 'msg' => 'Email already verified.', 'data' => $user], 200);
        }
        $email = $user->email;
        ...
        return response()->json(['success' => true, 'msg' => 'OTP sent to your email address. Verify it.', 'data' => $user], 200);
    }
    ...
    //     if (!$otpData) {
    //         return response()->json(['error' => true, 'msg' => 'You entered a wrong OTP.']);
    //     }

    //     $otpCreationTime = Carbon::parse($otpData->created_at);
    //     $currentTime = Carbon::now();

    //     if ($currentTime->diffInSeconds($otpCreationTime) <= 300) { 
    //         User::where('email', $email)->update([
    //             'is_email_verified' => 1
    //         ]);
    //         $otpData->delete();

    //         return response()->json(['success' => true, 'msg' => 'Email has been verified successfully.']);
    //     } else {
    //         return response()->json(['error' => true, 'msg' => 'Your OTP has expired.']);
    //     }
    // }
    ...
    public function resendOtp(Request $request)
    {
        ...
        if (!$otpData) {
            return response()->json(['success' => false, 'msg' => 'OTP data not found']);
        }

        $currentTime = now()->timestamp;
        $otpCreationTime = $otpData->created_at->timestamp; 

        if ($currentTime <= $otpCreationTime + 65) {
    ...
    public function passwordResetOtp($user)
    {
        $otp = rand(1000,9999);
        $time = time();

        ForgetPasswordVerification::updateOrCreate(
            ['email' => $user->email],
            [
            'email' => $user->email,
            'otp' => $otp,
        ...
        $user->notify(new otpPasswordReset($otp));
    }
    ...
    public function passwordVerification(Request $request)
    {
        ...
        $otpData = ForgetPasswordVerification::where('email', $request->email)
                                            ->where('otp', $request->otp)
                                            ->first();
        if (!$otpData) {
            return response()->json(['error' => true, 'msg' => 'You entered wrong OTP']);
        }

        $currentTime = time();
        $otpCreationTime = $otpData->created_at->timestamp;

        if ($currentTime >= $otpCreationTime && $currentTime <= ($otpCreationTime + 10 * 60 + 5)) {
            return response()->json(['success' => true, 'msg' => 'Your email has been verified. Please proceed to reset your password.']);
        } else {
            return response()->json(['error' => true, 'msg' => 'Your OTP has been expired. Try again later.']);
        }
    }
    ...
    public function superAdminLogin(Request $request): JsonResponse
    {
        ...
        $token = $user->createToken('SuperAdminAuth')->accessToken;

        return response()->json([
            'message' => 'Super admin logged in as user successfully',
            'token' => $token,
            'user' => $user,
        ], 200);
    }
