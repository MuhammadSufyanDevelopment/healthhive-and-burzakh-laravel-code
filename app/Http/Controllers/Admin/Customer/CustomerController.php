...
class CustomerController extends BaseController
{
    ...
    function getVerifyDocumntsView($id){
        $users = User::find($id);
        return view('admin-views.customer.verify-documents', compact('users'));
    }

    // public function viewDocuments($id){
    //     $users = User::find($id);
    //     return view('admin-views.customers.verify_documents', compact('users'));
    // }

    public function updateDocumentVerificationAccept($id){
        $user = User::find($id);
        if($user){
            User::update(['documents_verified',1]);
            return redirect()->back()->with('success','Document Verification Completed and Email sent to the user.');
        }
    }

    public function updateDocumentVerificationReject($id){
        $user = User::find($id);
        if($user){
            User::update(['documents_verified',2]);
            return redirect()->back()->with('success','Document Verification Completed and Email sent to the user.');
        }
    }


    public function updateDocumentVerification($id, $status)
    {
        ...
        $user->update(['documents_verified' => $status]);

        if ($status == 1) {
            $user->update(['verification_time'=>now()]);
            $user->notify(new DocumentAcceptedNotification());
            $message = 'Document Verified and Email sent to the user.';
        } else {
            $user->update(['verification_time'=>now()]);
            $user->notify(new DocumentRejectedNotification());
            $message = 'Document Rejected and Email sent to the user.';
        }
